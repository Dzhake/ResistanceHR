...
namespace ResistanceHR.Patches
{
    ...
    public static class P_SkillPoints
    {
        ...
        {
            if (amount == 0) // Means string not picked up
            {
        ...
        //WARNING:
        // 1. This is an IEnumerator patch, so it won't run on its own
        // You'll also need to patch it in a very specific way. See SORCE for how to patch an IEnumerator.
        // 2. It's very rote and has a lot of pre-implementation notes scattered throughout
        // Don't delete it until you're sure everything is implemented.
        private static IEnumerator AddPointsLate_Enumerator(string pointsType, int extraNum, SkillPoints __instance, Agent ___agent)
        {
            ...
            switch (text)
            {
                ...
                case "ArrestedPointsInnocent":
                    if (___agent.HasTrait(VanillaTraits.TheLaw) && !___agent.oma.superSpecialAbility ||
                            ___agent.HasTrait<Very_HardOn_Yourself>()) // TODO is new
                    {
                        if (___agent.HasTrait(VanillaTraits.Crooked))
                            xpReward = 0;
                        else if (___agent.HasTrait(VanillaTraits.Crooked2))
                            xpReward = -40;
                        else
                            xpReward = -80;
                    }
