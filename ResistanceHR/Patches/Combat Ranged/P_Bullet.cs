    ...
    public static class P_Bullet
	{
		...
		public static bool LateUpdateBullet_Prefix(Bullet __instance, Transform ___bulletSpriteTr)
		{
			...
			if (Time.timeScale != 0f)
			{
				...
				float maxBulletDistance = 13.00f;
					...
					__instance.bulletType != bulletStatus.FireExtinguisher)
				{
					foreach (T_BulletModification trait in __instance.agent.GetTraits<T_BulletModification>())
						maxBulletDistance *= trait.BulletRangeMultiplier;
				}
								...
								(GC.serverPlayer || __instance.agent.isPlayer != 0))
							{
								...
								if (__instance.agent.controllerType == "Keyboard")
								{
									if (GC.sessionDataBig.trackpadMode)
									{
										...
										v = __instance.agent.agentCamera.actualCamera.ScreenCamera.ScreenToWorldPoint(UnityEngine.Input.mousePosition);
								}
								else if (__instance.agent.target.AttackTowardTarget())
		...
		/// <summary>
		/// Bullet Modification Speed & Damage
		/// </summary>
		/// <param name="__instance"></param>
		...
		public static void SetupBullet_Postfix(Bullet __instance)
		{
			...
			{
				IEnumerable<T_BulletModification> traitList = __instance.agent.GetTraits<T_BulletModification>();
				...
				//foreach (T_BulletModification trait in traitList)
    //            {
				//	damage *= trait.BulletDamageMultiplier;
				//}

				//__instance.damage = Mathf.Clamp((int)damage, 1, 9999);
			}
		}
	}
}