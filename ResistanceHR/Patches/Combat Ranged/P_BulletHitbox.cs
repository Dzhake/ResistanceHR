...
namespace ResistanceHR.Patches.Combat_Ranged
{
    ...
    public static class P_BulletHitbox
	{
		...
		/// Bullet Range
		/// </summary>
		/// <param name="playfieldObject"></param>
		/// <param name="__instance"></param>
		/// <param name="__result"></param>
		/// <param name="___hitsAlloc"></param>
		/// <returns></returns>
		...
		public static bool HasLOSBullet_Prefix(PlayfieldObject playfieldObject, BulletHitbox __instance, ref bool __result, ref RaycastHit2D[] ___hitsAlloc)
		{
			...
			if (__instance.myBullet.agent != null && __instance.myBullet.agent.GetTraits<T_BulletModification>().Any())
			{
				float maxBulletRange = T_BulletModification.GetBulletRange(__instance.myBullet);
						...
						bulletType != bulletStatus.ResearchGun)
					{
							...
							objectReal != myBullet.objectReal)
						{
							...
							if (!objectReal.bulletsCanPass)
							{
								...
								// Sniper_2
								if (myBullet.agent.HasTrait<Sniper2>() &&
		...
		public static bool OnTriggerEnter2D_Prefix(Collider2D other, BulletHitbox __instance)
		{
			...
			if (other.CompareTag("ObjectRealSprite") && __instance.myBullet.agent != null &&
				__instance.myBullet.agent.HasTrait<Sniper2>() && __instance.myBullet.agent.hiddenInObject != null)
			{
				try
				{
